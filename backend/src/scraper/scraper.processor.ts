...
export class ScraperProcessor extends WorkerHost {
  ...
  async process(job: Job<any, any, string>): Promise<any> {
    ...
    this.logger.log(
      `Procesando tarea de scraping para la empresa ${empresaId}...`,
    );
    ...
    try {
      ...
      this.logger.log(
        `Tarea completada exitosamente para la empresa ${empresaId}.`,
      );

      if (notifyUserId) {
        this.eventsGateway.emitToUser(notifyUserId, 'sync-finished', {
          empresaId,
          status: 'success',
        });
      }
      ...
      this.logger.error(
        `Falló la tarea de scraping para la empresa ${empresaId}`,
        error.stack,
      );

      if (notifyUserId) {
        this.eventsGateway.emitToUser(notifyUserId, 'sync-error', {
          empresaId,
          status: 'error',
